...
class GenerationRequest(BaseModel):
    ...
    # "auto" leaves the language token unset so the model infers it from the
    # text and the voice-clone reference. That matters for accents the model
    # has no token for: Qwen3-TTS carries a single `portuguese` id, so asking
    # for it explicitly pulls output toward the European variant regardless of
    # what the reference speaker sounds like.
    language: str = Field(
        default="en", pattern="^(auto|zh|en|ja|ko|de|fr|ru|pt|es|it|he|ar|da|el|fi|hi|ms|nl|no|pl|sv|sw|tr)$"
    )
    seed: int | None = Field(None, ge=0)
    model_size: str | None = Field(default="1.7B", pattern="^(1\\.7B|0\\.6B|1B|3B)$")
    instruct: str | None = Field(None, max_length=500)
    engine: str | None = Field(
        default="qwen", pattern="^(qwen|qwen_custom_voice|luxtts|chatterbox|chatterbox_turbo|tada|kokoro|gemini)$"
    )
...
class CaptureSettingsResponse(BaseModel):
    ...
    stt_engine: Literal["whisper", "gemini"] = "whisper"
...
class CaptureSettingsUpdate(BaseModel):
    ...
    stt_engine: Literal["whisper", "gemini"] | None = None
    language: str | None = None
...
class ProviderSettingsResponse(BaseModel):
    """Remote-provider status and defaults. Credentials are never returned."""

    gemini_key_present: bool
    gemini_key_source: Literal["env", "stored"] | None = None
    gemini_key_hint: str | None = None
    gemini_stt_model: str
    gemini_stt_mode: Literal["verbatim", "smart"]
    gemini_stt_diarization: bool
    gemini_stt_timestamps: bool
    gemini_stt_language_codes: list[str]
    gemini_stt_custom_vocabulary: list[str]
    gemini_tts_model: str
    gemini_tts_style_prompt: str | None = None


class ProviderSettingsUpdate(BaseModel):
    """Partial provider update. Explicit null clears the stored Gemini key."""

    gemini_api_key: str | None = Field(None, max_length=500)
    gemini_stt_model: str | None = Field(None, max_length=100)
    gemini_stt_mode: Literal["verbatim", "smart"] | None = None
    gemini_stt_diarization: bool | None = None
    gemini_stt_timestamps: bool | None = None
    gemini_stt_language_codes: list[str] | None = Field(None, max_length=23)
    gemini_stt_custom_vocabulary: list[str] | None = Field(None, max_length=1000)
    gemini_tts_model: str | None = Field(None, max_length=100)
    gemini_tts_style_prompt: str | None = Field(None, max_length=1000)
...
class MCPClientBindingResponse(BaseModel):
    ...
    default_engine: str | None = Field(
        None,
        pattern="^(qwen|qwen_custom_voice|luxtts|chatterbox|chatterbox_turbo|tada|kokoro|gemini)$",
    )
...
class SpeakRequest(BaseModel):
    ...
    language: str | None = Field(
        None,
        pattern="^(auto|zh|en|ja|ko|de|fr|ru|pt|es|it|he|ar|da|el|fi|hi|ms|nl|no|pl|sv|sw|tr)$",
    )
...
class CaptureReadinessResponse(BaseModel):
    """Backend gates required before the global hotkey can fire.

    ``llm`` is omitted when Gemini Smart transcription performs cleanup itself;
    local Whisper and Gemini Verbatim still require the selected refinement LLM.
    """

    stt: ModelReadiness
    llm: ModelReadiness | None = None
