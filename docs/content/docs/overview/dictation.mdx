...
**Press `Esc` while recording to throw the capture away.** The audio is
discarded in the browser process — nothing is uploaded, no capture row is
created, and nothing pastes into your app. The pill disappears immediately.

`Esc` works from any app, not just Voicebox: while a recording is running,
Voicebox opens a second read-only keyboard tap alongside the chord tap, so
the pill can pick the key up without ever taking focus. The tap closes the
moment the capture ends, and it never swallows the key — `Esc` still reaches
whatever you were typing in.

Cancel is only available while the pill reads `Recording`. Once transcription
starts the capture is already on its way to the Captures tab; delete it from
there instead.
...
| State | What it shows |
|---|---|
| `Recording` | Live waveform + elapsed time, and an `esc` chip you can click to cancel. |
