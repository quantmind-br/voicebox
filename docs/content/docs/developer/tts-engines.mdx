...
Remote engines such as Gemini use the same `TTSBackend` generation contract but do not
ship local weights. They therefore contribute no `ModelConfig`, never appear in model
management, and require no download or HuggingFace cache entry. Readiness is based on
provider credentials instead.

Provider credentials and engine-specific defaults are resolved in the service or route
layer and passed as an optional `options` dictionary. `generate_chunked()` forwards that
dictionary to `backend.generate(..., options=options)` without storing mutable options on
the singleton backend. This keeps concurrent requests isolated.

An HTTP-only integration using dependencies already in Voicebox still needs PyInstaller
hidden imports for its dynamically imported backend and client modules, but no
`--collect-all`, package metadata, runtime data files, or native-path setup.
